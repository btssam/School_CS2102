...
public abstract class StrategyBTAbs<E> implements IBTStrategy<E> {
    ...
    /**
     * A generic removal process defined in terms of addition by: - searching for the element - replacing the node with
     * that element at the root with the left subtree merged into the right using repeated addInt()
     *
     * @param elt
     *            the element to remove
     * @param b
     *            the tree from which to remove it
     *
    ...
    public IBinTree<E> removeElt(E elt, IBinTree<E> b) {
        if (b.isEmpty()) {
            ...
            return new BTNode<>(b.getRoot(), this.removeElt(elt, b.getLeft()), this.removeElt(elt, b.getRight()));
        }
    }
     ...
     *
     * @param from
     *            the tree whose elements we are taking from
     * @param to
     *            the tree that we are building up
     *
