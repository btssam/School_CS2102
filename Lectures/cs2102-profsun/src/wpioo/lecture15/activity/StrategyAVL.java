...
public class StrategyAVL<E extends Comparable<E>> extends StrategyBST<E> {
    ...
    public IBinTree<E> addElt(E elt, IBinTree<E> b) {
        IBinTree<E> newTree;
        if (b.isEmpty()) {
            newTree = new BTNode<>(elt, new BTEmpty<>(), new BTEmpty<>());
        } else if (elt.compareTo(b.getRoot()) < 0) {
            newTree = new BTNode<>(b.getRoot(), this.addElt(elt, b.getLeft()), b.getRight());
        } else {
            newTree = new BTNode<>(b.getRoot(), b.getLeft(), this.addElt(elt, b.getRight()));
        }

        IBinTree<E> mt = new BTEmpty<>();
        // left-left rotation
        if (newTree.getLeft().height() == 2 && newTree.getLeft().getLeft().height() == 1
                && newTree.getRight().height() == 0) {
            return new BTNode<>(newTree.getLeft().getRoot(), newTree.getLeft().getLeft(),
                    new BTNode<>(newTree.getRoot(), mt, mt));
        }
        // left-right rotation
        else if (newTree.getLeft().height() == 2 && newTree.getLeft().getRight().height() == 1
                && newTree.getRight().height() == 0) {
        ...
        else if (newTree.getRight().height() == 2 && newTree.getRight().getRight().height() == 1
                && newTree.getLeft().height() == 0) {
            return new BTNode<>(newTree.getRight().getRoot(), new BTNode<>(newTree.getRoot(), mt, mt),
                    newTree.getRight().getRight());
        }
        // right-left
        else if (newTree.getRight().height() == 2 && newTree.getRight().getLeft().height() == 1
                && newTree.getLeft().height() == 0) {
            return new BTNode<>(newTree.getRight().getLeft().getRoot(), new BTNode<>(newTree.getRoot(), mt, mt),
                    new BTNode<>(newTree.getRight().getRoot(), mt, mt));
        } // balanced by default
        else {
            return newTree;
        }
    ...
    public IBinTree<E> removeElt(E elt, IBinTree<E> b) {
        ...
        if (b.isEmpty()) {
            ...
            if (b.size() == 1) {
                ...
                return new BTNode<>(leafElt, b.getLeft(), removeElt(leafElt, b.getRight()));
            }
        } else {
            // https://www.geeksforgeeks.org/deletion-in-an-avl-tree/
            IBinTree<E> newTree;
            if (elt.compareTo(b.getRoot()) < 0) {
                newTree = new BTNode<>(b.getRoot(), this.removeElt(elt, b.getLeft()), b.getRight());
            } else {
                newTree = new BTNode<>(b.getRoot(), b.getLeft(), this.removeElt(elt, b.getRight()));
            }
            ...
            if (balance > 1) {
                ...
                if (newTree.getLeft().height() > newTree.getRight().height()
                    ...
                    t4 = newTree.getRight();
                    return new BTNode<>(y, new BTNode<>(x, t1, t2), new BTNode<>(z, t3, t4));
                } // left-right
                    ...
                    t4 = newTree.getRight();
                    return new BTNode<>(x, new BTNode<>(y, t1, t2), new BTNode<>(z, t3, t4));
                } // right-right
                    ...
                    return new BTNode<>(y, new BTNode<>(z, t1, t2), new BTNode<>(x, t3, t4));
                } // right-left
                    ...
                    return new BTNode<>(x, new BTNode<>(z, t1, t2), new BTNode<>(y, t3, t4));
                } else {
    ...
    /**
     * Used as a helper method used to get the minimum value from a subtree
     *
     * @param b
     *            a subtree
     *
