...
public class GradeCalculator {
    ...
    public double hwPercentage(int hwPoints, int hwsAttempted) {
        return hwsAttempted > 0 ? hwPoints / (POINTS_PER_HOMEWORK * hwsAttempted) : 0.0;
    }
    ...
    public String fuzzyLetterGrade(int quiz1, int quiz2, int quiz3, int quiz4, int hwPoints, int hwsAttempted) {
        double avg = (this.quizPercentage(quiz1, quiz2, quiz3, quiz4) + this.hwPercentage(hwPoints, hwsAttempted))
                / 2.0;
    ...
    public double labPercentage(int labPoints, int labsAttempted, int labsAttended) {
        double answer;
        if (labsAttempted > 0) {
            answer = (labPoints + (labsAttended * LAB_ATTENDANCE_EC)) / (POINTS_PER_LAB * labsAttempted);
        } else {
            answer = 0.0;
        }
